...
async def get_factory_metrics(
    ...
    # TODO improve documentation. check conversion of datetimes/dates.
    """
...
async def get_customer_group_data(
    ...
    product orders.
    """
    db_records = await crud.get_customer_groups_data_db(cursor, date, customer_group_id)
    result = dict()
...
@app.post("/candidate_plans")  # submit selected plan
async def select(plan_id: int, cursor: db.Cursor = Depends(db.get_cursor)):
