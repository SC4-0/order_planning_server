...
async def optimize(
    ...
    cursor: db.Cursor = Depends(db.get_cursor),
):  # returns all plan_ids generated by engine
    """
...
@app.get(
    "factory_parameters"
)  # get factory address, production hours, production rate,
async def get_factory_parameters():
    db_records = await crud.get_factory_parameters_db()
    result = dict()
    return result
...
async def post_selected_plan(plan_id: int, cursor: db.Cursor = Depends(db.get_cursor)):
    db_records = await crud.select_plan_db(cursor)
    result = dict()
...
# TODO: split into overall planned metrics, factory-specific metric for optimization results

# get allocation from /allocations


# @app.get("/factory_targets/{plan_id}", response_model=schemas.FactoryTargetsResponse)
@app.get("/factory_targets/{plan_id}")
async def get_factory_targets(plan_id: int, cursor: db.Cursor = Depends(db.get_cursor)):
    """
